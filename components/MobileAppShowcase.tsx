...
export default function MobileAppShowcase() {
  ...
  const currentIndex = APP_SCREENS.findIndex((s) => s.id === activeScreen);
  const currentScreen = APP_SCREENS[currentIndex] || APP_SCREENS[0];

  // Auto-cycle screens every 5 seconds
  useEffect(() => {
    autoPlayRef.current = setInterval(() => {
      setActiveScreen((prev) => {
        const idx = APP_SCREENS.findIndex((s) => s.id === prev);
        return APP_SCREENS[(idx + 1) % APP_SCREENS.length].id;
      });
    }, 5000);
    return () => {
  ...
  const handleUserInteraction = (screenId: string) => {
  ...
  const handleDragEnd = (_: unknown, info: { offset: { x: number }; velocity: { x: number } }) => {
    ...
    if (info.offset.x < -swipeThreshold || info.velocity.x < -500) {
      // Swipe left = next
      const next = APP_SCREENS[(currentIndex + 1) % APP_SCREENS.length];
      handleUserInteraction(next.id);
    } else if (info.offset.x > swipeThreshold || info.velocity.x > 500) {
      // Swipe right = previous
      const prev = APP_SCREENS[(currentIndex - 1 + APP_SCREENS.length) % APP_SCREENS.length];
      handleUserInteraction(prev.id);
    }
  };
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          ...
          >
              ...
              <div className="absolute inset-2 rounded-[2.5rem] overflow-hidden">
                <AnimatePresence mode="wait">
                  <motion.div
                    ...
                    className={`h-full bg-green-600 p-6 flex flex-col`}
                  >
                    {/* Status bar */}
            ...
            {/* Dot indicators */}
            <div className="flex justify-center gap-2 mt-6" role="tablist" aria-label="App screens">
